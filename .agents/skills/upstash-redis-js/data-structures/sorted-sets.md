...
const bottom2WithScores = await redis.zrange('leaderboard', 0, 1, {
  withScores: true,
});
...
const top2WithScores = await redis.zrange('leaderboard', 0, 1, {
  withScores: true,
  rev: true,
});
...
const midRangeDesc = await redis.zrange('leaderboard', 250, 150, {
  rev: true,
  byScore: true,
});
// ["player2", "player3"]
...
const lastHour = await redis.zrange('events', now - 3600000, now, {
  byScore: true,
});
