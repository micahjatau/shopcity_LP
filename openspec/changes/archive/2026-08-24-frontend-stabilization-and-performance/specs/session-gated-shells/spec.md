...
- **AND** descendant routes consume the shared context rather than starting duplicate bootstrap requests
...
- **AND** task surfaces do not expose supervisor/admin-only PII or controls
...
- **AND** shared workspaces receive explicit capabilities rather than inferring role from pathname
...
- **AND** all consumers receive the same session-owned value
...
- **AND** the user receives a clear recovery action
