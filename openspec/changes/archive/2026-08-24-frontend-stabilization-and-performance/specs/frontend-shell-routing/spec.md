...
The frontend SHALL use one canonical navigation registry to drive role-based shell visibility, active state, route metadata, and primary workflow links.
...
#### Scenario: Shared workspace links remain role-neutral

- **GIVEN** a shared Transaction, Customer, or Card workspace renders an action link
- **WHEN** the user activates that link
- **THEN** it resolves through the canonical registry or an explicit capability-provided destination
- **AND** the shared workspace does not hard-code a supervisor or admin route
...
- **AND** customer balances, roles, and approvals remain backend-authoritative
