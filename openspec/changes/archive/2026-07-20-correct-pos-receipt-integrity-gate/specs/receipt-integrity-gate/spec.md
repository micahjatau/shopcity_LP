...
### Requirement: Physical POS receipt numbers must replace the generated business receipt number

The system MUST require a physical POS receipt number when capturing a receipt, MUST remove the generated business `receiptNumber` from the receipt contract, and MUST treat the normalized weekly physical receipt identity as the uniqueness boundary.
...
### Requirement: Receipt capture must be tied to active cashier and device context
