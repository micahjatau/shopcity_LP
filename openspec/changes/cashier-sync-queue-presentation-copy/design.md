...
- Compose the page heading with the shared `CashierPageHeader` and `cashier-route-header` styles used by Find Customer, Capture Purchase, and Redeem Credit; keep one clear H1, concise explanatory copy, and visually subordinate section headings/body text.
...
- Use cashier-facing language rather than implementation terminology where the meaning remains precise (`Saved purchases`, `No saved purchases`, and `Sign in again`), and retain exact distinction between readable local records, device identity, and confirmed sync results.
