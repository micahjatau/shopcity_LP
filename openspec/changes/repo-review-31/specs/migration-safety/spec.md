...
The system MUST verify deployed Prisma migrations create the expected indexes and constraints, and the actual shared database MUST be restored into an isolated verification database where `_prisma_migrations` rows, committed migration folders, checksums, `pg_constraint`, `pg_trigger`, `pg_proc`, and representative historical effects are compared before release.
...
- **THEN** `_prisma_migrations` rows, checksums, committed migration folders, and expected SQL objects are compared and any drift fails verification

#### Scenario: Schema drift is detected

- **WHEN** the SQL migration output does not match the Prisma model, the restored SQL objects, or the shared migration ledger
...
### Requirement: Repair migrations fail closed on missing prerequisites

The system MUST either recreate every required SQL function and trigger used by a forward repair migration or fail immediately with an explicit inventory of missing dependencies.
...
- **THEN** the migration fails closed and lists the missing prerequisites
