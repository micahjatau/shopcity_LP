...
### Requirement: Release evidence proves immutable candidate provenance

Release evidence SHALL independently verify candidate commit SHA, workflow head SHA, successful required conclusions, deployment artifact SHA, protected-branch gates, topology/project decisions, and authenticated checkout benchmark artifacts.

#### Scenario: Candidate evidence is reviewed

- **WHEN** a release candidate is certified
- **THEN** every required artifact references the same immutable candidate SHA
- **AND** certification fails for missing, stale, mixed-SHA, or unsuccessful evidence
