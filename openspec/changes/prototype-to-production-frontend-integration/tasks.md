...
- [ ] 4.1 Make scanner/card-serial input the first-class lookup entry point and wire it to `GET /cards/lookup/{serial}`.
- [ ] 4.2 Ensure directory search can discover customers but cannot unlock financial actions without verified card context.
...
- [ ] 4.4 Ensure Earn submits the authoritative card serial, integer-kobo amount, receipt number, CSRF, and idempotency headers through the generated client.
- [ ] 4.5 Ensure Redeem submits the authoritative card serial and renders server-authoritative balance, policy, approval, insufficient-balance, and success outcomes.
...
- [ ] 4.7 Make normal cashier login/bootstrap expose usable device, branch, actor, and session context for Offline Earn; remove manual identity fields from production UI.
...
- [ ] 5.6 Implement real logout, session-expiry cleanup, and recovery routing; remove dead `#logout` and fake password-recovery behavior.
...
- [ ] 6.1 Keep the cashier-today overview bounded and label it as recent/loaded activity; defer full transaction history to a separate API proposal.
...
- [ ] 6.3 Remove browser-only pagination and ensure the UI does not claim full history beyond the bounded feed.
...
- [ ] 9.1 Choose one documented prototype reference location during migration.
- [ ] 9.2 Remove duplicate root-level HTML/CSS/JS and `.artifact.json` copies after React parity, or explicitly exclude them from production and document why they remain.
- [ ] 9.3 Ensure generated OpenAPI/client artifacts are regenerated by CLI after contract changes rather than hand-edited.
- [ ] 9.4 Add a repository check preventing duplicate production frontend implementations and sensitive prototype artifacts from shipping.
